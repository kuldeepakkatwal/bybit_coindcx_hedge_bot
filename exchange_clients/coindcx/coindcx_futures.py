...
class CoinDCXFutures:
    ...
    def edit_order(
        ...
        """
        Edit an existing futures order (USDT margined futures only according to docs)

        Args:
            order_id: Order ID to edit
            price: New price for the order
            quantity: New total quantity (optional, keeps existing if not provided)
        ...
        Note: According to CoinDCX documentation, this endpoint is currently
        limited to USDT margined futures. May not work for INR margined orders.
        """
        body = {
            "id": order_id,
            "price": price
        ...
        if quantity is not None:
            body["total_quantity"] = quantity
        if take_profit_price is not None:
            body["take_profit_price"] = take_profit_price
        if stop_loss_price is not None:
            body["stop_loss_price"] = stop_loss_price

        result = self._make_request('POST', '/exchange/v1/derivatives/futures/orders/edit', body)
        logger.info(f"Order {order_id[:8]}... edited: price=${price}")
        return result
